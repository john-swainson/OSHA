...
import { AlertService, OshaService, AuthenticationService, DashboardService } from '../../_services';
...
@Component({
  ...
  templateUrl: './dashboard_type.component.html',
  styleUrls: ['./dashboard_type.component.scss']
})
export class DashboardTypeComponent implements OnInit {
  ...
  filter: string = '';
  //===== filtered objects ====
  filtered_objects: Array<any> = [];
  filtered_labels: Array<{key: string, label: string}> = [];
  ...
  constructor(private alertService: AlertService, public oshaService: OshaService,
      ...
      this.tableName = this.remove__c(this.data_type);
